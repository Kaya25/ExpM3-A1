...
```{r}
library(stringr)
LU$VISIT = str_extract(LU$VISIT,"\\d")      # extracts only digits (d) 
...
```{r}
# Rename variables
   # note: rename has issues with dplyr
library(plyr)
...
demo_sub = select(demo, SUBJ, VISIT, Ethnicity, Diagnosis, Gender, Age, ADOS,  nonVerbalIQ, verbalIQ)

LU_sub = select(LU, SUBJ, VISIT, MOT_MLU, MOT_LUstd, CHI_MLU, CHI_LUstd)
...
```{r}
temporal = merge(LU_sub, token_sub, by=c("SUBJ", "VISIT"))

total = merge(temporal, demo_sub, by=c("SUBJ", "VISIT"))
...
d = merge(total, select(total_sub, SUBJ, ADOS, nonVerbalIQ, verbalIQ), by="SUBJ")

# Delete old columns
d = subset(d, select = -c(ADOS.x, nonVerbalIQ.x, verbalIQ.x))

# Rename the columns
d = plyr::rename(d, c(ADOS.y="ADOS"))
d = plyr::rename(d, c(nonVerbalIQ.y="nonVerbalIQ"))
d = plyr::rename(d, c(verbalIQ.y="verbalIQ"))
...
* In order to make it easier to work with this nice, clean dataset in the future, it is practical to make sure the variables have sensible values. E.g. right now gender is marked 1 and 2, but in two weeks you will not be able to remember, which gender were connected to which number, so change the values from 1 and 2 to M and F in the gender variable. For the same reason, you should also change the values of Diagnosis from A and B to ASD (autism spectrum disorder) and TD (typically developing). 
Tip: Google "how to rename levels in R".


```{r}
# Turn SUBJ to numeric
d$SUBJ = as.numeric(as.factor(d$SUBJ))  # as.factor changes names to numbers
...
d$Diagnosis = factor(d$Diagnosis, levels = c("A", "B"), labels = c("ASD", "TD"))
...
d_temp = d %>%          # %>% = pipes (in library(magrittr)); chains functions together
  # Look at one subject at a time
  group_by(SUBJ) %>%   
  # Only choose the ones where the mean of all CHI_MLU scores >2.7
...
d_temp = d %>%
  group_by(SUBJ) %>%
  filter(sum(VISIT) < 21)   # sum of the visits for those who completed all trials is 21, so we're looking for those who have <21
...
d_temp = d %>%
  ...
  filter(VISIT ==6) %>% 
  # Arrange trials in descending order for tokens_CHI
  arrange(-tokens_CHI) 
...
d_temp = d %>%
  ...
  filter(VISIT ==1) %>% 
  # Arrange trials in ascending order for tokens_CHI
  arrange(tokens_CHI) 
...
```{r}
# For each visit, find the mean number of words spoken by all the kids
d = d %>% 
  group_by(VISIT) %>% 
...
```{r}
# For each child, find the mean number of words spoken at a visit
d = d %>% 
  group_by(SUBJ) %>% 
...
We preferably don't want to lose any data. Using only average values to describe the language production means that while not technically losing data, we could be losing information. Averages don't capture everything and could be inaccurate description of data.
